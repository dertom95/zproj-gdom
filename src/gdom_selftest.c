...
// Tests for draft public classes:
    { "gdom_server", gdom_server_test, false, true, NULL },
    { "gdom_client", gdom_client_test, false, true, NULL },
...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef GDOM_BUILD_DRAFT_API // selftest is still in draft
        else
            gdom_private_selftest (verbose, item->subtest);
#endif // GDOM_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        ...
        if (!test->subtest)
            ...
            gdom_private_selftest (verbose, test->subtest);
#endif // GDOM_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);

...
IF %1.==--help. (
    ...
    ECHO    --without-gdomd         do not build gdomd.exe
    ECHO    --without-gdom_selftest  do not build gdom_selftest.exe
    GOTO :eof
)
ECHO Configuring Global Domination...
...
ECHO. >> platform.h
ECHO #define GDOM_HAVE_WINDOWS 1>> platform.h
...
IF EXIST "..\..\..\libzmq" (
    ...
    ECHO Global Domination cannot build without libzmq
...
IF EXIST "..\..\..\czmq" (
    ...
    ECHO Global Domination cannot build without czmq
...
:with_draft
    ...
    ECHO #define GDOM_BUILD_DRAFT_API 1>>platform.h
    GOTO end_draft
:no_draft
    ECHO Building without draft API (stable + legacy API)
    ECHO #undef GDOM_BUILD_DRAFT_API 1>>platform.h
:end_draft
ECHO. >> platform.h
